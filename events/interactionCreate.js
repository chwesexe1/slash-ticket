...
const asil = require("croxydb");
const asildb = require("orio.db")
...
module.exports = async(client, interaction) => {
  ...
  if(interaction.isButton()) {
    ...
    if(interaction.customId === `onayla_${interaction.user.id}`) {
      ...
      asil.set(`ticketKatagor_${guild.id}`, { category:  category.id, log: ticketLog.id });
      asil.set(`ticketSistem_${guild.id}`, { isOpen: true });
  ...
  if(interaction.customId === `ticketAc_${interaction.guild.id}`) {
    ...
    const ticketKatagor = asil.fetch(`ticketKatagor_${guild.id}`);
    const ticketSistem = asil.fetch(`ticketSistem_${guild.id}`);
    const ticketKullanıcı = asildb.fetch(`ticketKullanıcı_${user.id}${guild.id}`);
    ...
    asildb.set(`ticketKullanıcı_${user.id}${guild.id}`, { isTicketAc: true, channelId: channel.id, guildId: guild.id, date: Date.now() });
    asildb.set(`ticketKanalKullanıcı_${channel.id}${guild.id}`, { userId: user.id, channelId: channel.id, guildId: guild.id });
  ...
  if(customId === `ticketKapat_${guild.id}${interaction.channel.id}`) {
    ...
    const ticketKullanıcı = asildb.fetch(`ticketKanalKullanıcı_${interaction.channel.id}${guild.id}`);
    if(!ticketKullanıcı) {
    ...
    if(ticketKullanıcı) {
      ...
      const channel = await client.channels.cache.get(asil.fetch(`ticketKatagor_${guild.id}`).log)
    ...
    channel.send({ embeds: [ticketUserClose] })
     ...
     asildb.delete(`ticketKullanıcı_${ticketKullanıcı.userId}${guild.id}`)
     asildb.delete(`ticketKanalKullanıcı_${interaction.channel.id}${guild.id}`); 
  ...
  if(interaction.isChatInputCommand()) {
    ...
    readdirSync('./commands').forEach(f => {
      ...
      if(interaction.commandName.toLowerCase() === cmd.name.toLowerCase()) {
        return cmd.run(client, interaction, asil);
      }
});
}
  }